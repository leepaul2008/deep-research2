...
| 规则 | 说明 | 在哪拦截 |
|:----|:-----|:---------|
| **Scrapling 为默认抓取工具** | 安装环节已确保 Scrapling 就绪，Task 2 直接使用 Scrapling 批量抓取；单 URL 失败则回退 webfetch | ⚡ 阻断点（Step 2→数据池构建） |
| **结构化事实** | 数据池使用 JSON facts[] 数组格式，非散装文本 | 阶段2 输出 |
| **跨源矛盾标记** | 不同来源矛盾数据在 controversies[] 数组中对齐 | 阶段2 数据池 |
| **来源稀缺** | 连续 3 不同域名 404 → 2 分钟上限 | 阶段2 |
| **不重复** | 每个新子问题前检查数据池已有内容 | 阶段2 Step 3（数据池构建） |

## 阶段3 规则（已合并至 Task 2 — 数据收集阶段内部处理预检）
...
| **三段式顺序** | 报告结构固定为：标题 → 元数据块 → `## 目录` → 正文各章，不可调换 | 阶段5 验收：第1行 `^# `，第2-6行含元数据和参考来源，之后紧跟 `## 目录` |
| **参考来源行** | 元数据后另起一行 `> **参考来源**：{主要来源} 等 · 共引用 N 个来源`，主要来源末尾加"等" | 阶段5 QA grep `^> \*\*参考来源\*\*` |
| **报告尾部** | 末尾固定包含 `## 参考来源`（全部引用机构及链接）和 `## 免责声明`（固定文本） | 阶段5 QA 检查末尾 20 行 |
| **TOC 标题唯一** | 报告有且仅有一个 `## 目录` 标题 | 阶段5 QA `grep -c "^## 目录"` 恰为 1 |
| **乱码零容忍** | data-pool.json 和最终报告均不得含替换字符（\ufffd）或典型 Mojibake 模式 | 阶段3 预检 + 阶段5 QA grep |
...
| **清理** | 调研结束后删除 TMPDIR | 阶段5 Step 3 |

## 反模式表

| 反模式 | 在哪拦截 |
|:-------|:--------|
| 数值量级错误（亿/billion 差 10x） | 阶段4 章节 agent 自检 + 阶段3 预检 |
