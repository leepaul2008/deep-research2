...
| 指标      | 数据                                                    |
| ------- | ----------------------------------------------------- |
| 报告长度    | 472 行 / 约 11,000 字                                    |
| 数据表     | 25 张，覆盖成因、影响、争议、治理、趋势等多个维度                            |
| 分析段落    | 142 段（每段含结论 + 数据 + 因果 + 判断）                           |
| 引用的独立机构 | 15+（IPCC、WWA、Nature、CEEW、Lancet Countdown、世界银行、IMD 等） |
| 反方观点    | 5 处（数据口径争议、低估幅度、国际方法论分歧等）                             |
| 数据收集    | ~30-45 秒                                              |
| 报告生成    | ~3-4 分钟                                               |
| 总耗时     | ~6 分钟                                                 |
...
| **oh-my-opencode（OMO）** | 提供 oracle / librarian 子 agent + 自动配置 MCP | `opencode plugins add oh-my-opencode`                                  |
| **Exa MCP**             | 网页搜索                                     | 由 OMO 自动配置                                                             |
| **Scrapling MCP**       | 网页全文抓取                                   | 由 OMO 自动配置                                                             |


> 本 skill 依赖 OMO 插件提供的 `oracle` 子 agent。如果没有 OMO，`/research` 命令无法执行。
...
⑥ 完成 → 文件保存到 ~/Desktop/调研/<主题>-YYYYMMDD-HHmmss.md
```
...
报告以 Markdown 格式保存到 `~/Desktop/调研/` 目录，文件名包含日期时间戳。可以用任何 Markdown 阅读器（Typora / Obsidian / VS Code 等）打开。
