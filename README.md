...
这个 skill 走完 **5 层流程**才交报告。不是搜完就出，是析→搜→验→写→再验。

## 谁适合用

**独立开发者**、**独立研究者**、**小团队**。  
没有预算买付费数据库，但想要专业级调研能力的人。
...
所有工具已内置，无需额外购买。系统采用 **自动级联搜索** 策略：Exa 语义搜索为主力，当某个子问题结果不足 3 条或数据年份偏旧时，自动触发备用源补强，逐层发现更多来源。
...
Exa 语义搜索（主力）
  ↓ Exa 结果不足或无果
DuckDuckGo Lite / Bing 国内版（英文/中文备用搜索）
Semantic Scholar（学术论文与研究报告）
GDELT Project（全球 50K+ 新闻源，实时事件）
```

**国内源（直连，无需代理，硬编码在 skill 指令中，自动尝试）：**
...
| 维基百科 | 中英文通用知识查询 |
| 知乎 | 中文问答/深度分析/用户评价 |
| 36氪 | 科技/商业新闻、创业公司、行业分析 |
| 澎湃新闻 | 时政/财经/科技深度报道 |
| 199IT | 行业研究报告聚合 |
| 艾瑞咨询报告库 | 互联网/消费/科技行业研报 |
| 东方财富 | 金融/市场/上市公司数据 |
| 国家统计局 | 宏观经济/人口/产业官方数据 |

> 除上述硬编码源外，备用搜索链路（DuckDuckGo/Bing/GDELT/Semantic Scholar）可**动态发现**任意其他网站，不限于此列表。
