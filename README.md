...
① 分析大纲 — 分析主题，生成调研框架和搜索计划
         ↓
② 采集数据 — Exa 搜索 + Scrapling 批量抓取 + 数据池提取 + 预检（合并完成）
         ↓
...
所有工具已内置，无需额外购买。系统采用 **自动级联搜索** 策略：Exa 为主力，当某个子问题结果不足 3 条或数据年份偏旧时，自动触发补强，逐层发现更多来源。整个流程如下：
...
① 搜索补强线    │  ② 已知源补强线
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DuckDuckGo Lite  │  百度百科 / 维基百科（术语背景）
Bing 国内版      │  知乎（问答分析）
Semantic Scholar │  199IT / 艾瑞咨询（行业报告）
GDELT 全球新闻   │  36氪 / 澎湃新闻（科技商业新闻）
                 │  东方财富（金融数据）
                 │  国家统计局（宏观数据）
```
...
| 命令                    | 用途          | 上限段落数 | 上限字数（字符）    | 参考耗时         |
| --------------------- | ----------- | --------- | ---------------- | ---------- |
| `/research 主题`        | standard 默认 | 8 段/章     | 12,000 字        | ~6–10 min  |
| `/research 主题 -quick` | 快速洞察        | 6 段/章     | 8,000 字         | ~5–8 min   |
| `/research 主题 -deep`  | 极致深度        | 10 段/章    | 25,000 字        | ~12–18 min |
...
```
① 分析大纲 — 分析主题，生成调研框架和搜索计划
② 采集数据 — Exa → Scrapling 批量抓取 → 数据池提取 → 预检验证（合并完成）
...
不需要。你只需要有一个大模型（API 或本地都行），其余全内置且免费。如果 Exa 触发了 rate limit，会自动走备用搜索，或免费注册 Exa 续用（每月 1,000 次，不绑卡）。
