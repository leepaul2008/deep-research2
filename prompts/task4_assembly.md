...
- 输出目录：`reports/{LANG}/`（语言子目录，由 `$LANG` 决定）
...
python {TOOLSDIR}/dr_tools.py assemble-report \
  ...
  --output reports/{LANG}/
```
（`reports/{LANG}/` 为默认输出目录，按语言分类；脚本自动根据报告标题+日期生成文件名）
...
☐ **路径核验**：报告保存路径属于默认语言子目录（`reports/{LANG}/`）或用户指定目录，两者之一；既非默认也非用户指定 → 标记"路径异常"不通过
☐ 时间戳：尾部时间与 date 匹配
☐ 反方观点：至少 1 处
☐ 跨来源归因一致
